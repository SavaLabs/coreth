...
ARG SAVANODE_VERSION

RUN mkdir -p $GOPATH/src/github.com/SavaLabs
WORKDIR $GOPATH/src/github.com/SavaLabs

RUN git clone -b SAVANODE_VERSION --single-branch https://github.com/SavaLabs/savannahnode.git
...
RUN go mod edit -replace github.com/SavaLabs/coreth=../coreth
...
COPY --from=builder /go/src/github.com/SavaLabs/savannahnode/build .
